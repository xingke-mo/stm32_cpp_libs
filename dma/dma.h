...
public:
    ...
    void init( uint32_t peripheral_base_addr,
    ...
    void setPeriphToConf( uint32_t base_addr,
                          uint32_t data_size = DMA_PeripheralDataSize_Byte,
                          uint32_t increment = DMA_PeripheralInc_Disable );

    void setMemoryToConf( uint32_t base_addr,
                          uint32_t data_size = DMA_MemoryDataSize_Byte,
                          uint32_t increment = DMA_MemoryInc_Enable );

    void setTransferToConf( uint32_t length,
                            uint32_t direction = DMA_DIR_PeripheralDST );

    void setFunctionToConf( uint32_t mode = DMA_Mode_Normal,
                            uint32_t priority = DMA_Priority_Low,
                            uint32_t m2m = DMA_M2M_Disable );

    void initWithConf( void );
